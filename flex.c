...
{
    size_t pos = dst - *buf;
    size_t len = (key ? strlen(key) + 5 : 0) + json_strlen(value) + 4;
...
json_double_flex(char**      buf,
                 ...
                 double      value)
{
    if (isnan(value)) { return json_str_flex(buf, siz, dst, key, "nan"); }
    if (isinf(value)) { return json_str_flex(buf, siz, dst, key, "inf"); }
...
{
    size_t pos     = dst - *buf;
    size_t b64_len = base64_encoded_size(n_data, base64_enc_modifier_padding);
    ...
    if (base64_encode((uint8_t*) dst,
                      data,
                      &b64_len,
                      &n_data,
                      base64_enc_modifier_padding)) {
        return NULL;
    }
